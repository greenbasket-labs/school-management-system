...
export default async function EditTermPage({
  ...
  const currentTermId = Number(termId);
  ...
  async function updateTerm(formData: FormData) {
    ...
    const duplicate = currentTerms.find(
      (item) =>
    ...
    if (wantsActive) {
      for (const existingTerm of currentTerms) {
        if (
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Update the term details for the{" "}
            ...
            <label className="flex cursor-pointer items-start gap-3">
              ...
              <span>
                ...
                <span className="mt-1 block text-xs text-slate-600">
                  Only one term can be active within an
                  academic session. Activating this term
                  will deactivate the current active term.
                </span>
              </span>
            </label>
          </div>
