...
function nowInstant() { return Temporal.Now.instant().toString(); }

function asDate(value: unknown) {
  if (value instanceof Date) return value;
  if (value && typeof value === "object" && "epochMilliseconds" in value) return new Date(Number((value as { epochMilliseconds: number }).epochMilliseconds));
...
function assertStaffType(userType: string) {
  if (!STAFF_TYPES.has(userType)) throw new Error("Only staff, teachers, cashiers, administrators, or owners can use staff attendance.");
}
...
async function getRecord(schoolId: number, userId: number, attendanceDate: Date) {
  ...
  return records.find((record) => record.schoolId === schoolId && record.userId === userId && dayKey(asDate(record.attendanceDate)) === key) ?? null;
}

export async function staffCheckIn(input: { schoolId: number; userId: number; attendanceDate?: Date }) {
  ...
  const lateMinutes = minutesFromStart(checkIn, configured.startHour, configured.startMinute);
  const status: StaffAttendanceStatus = lateMinutes > configured.graceMinutes ? "LATE" : "PRESENT";
  ...
  const record = existing
    ...
    : await db.orm.public.StaffAttendanceRecord.create({ schoolId: input.schoolId, userId: input.userId, attendanceDate: Temporal.Instant.fromEpochMilliseconds(attendanceDate.getTime()), checkInAt, checkOutAt: null, status, lateMinutes, missedCheckout: false, correctionReason: null, correctedByUserId: null });
  if (!record) throw new Error("Unable to save staff attendance.");

  await writeAuditLog({ schoolId: input.schoolId, userId: input.userId, action: existing ? "UPDATE" : "CREATE", entity: "StaffAttendance", entityId: String(record.id), newValue: { checkInAt, attendanceDate, status, lateMinutes, settings: configured } });
  return record;
}
...
export async function staffCheckOut(input: { schoolId: number; userId: number; attendanceDate?: Date }) {
  ...
  await writeAuditLog({ schoolId: input.schoolId, userId: input.userId, action: "UPDATE", entity: "StaffAttendance", entityId: String(existing.id), oldValue: { checkOutAt: existing.checkOutAt }, newValue: { checkOutAt } });
  return updated;
}
...
export async function getStaffAttendance(input: { schoolId: number; userId?: number; attendanceDate?: Date }) {
  ...
  return records.filter((record) => record.schoolId === input.schoolId && (input.userId === undefined || record.userId === input.userId) && (!key || dayKey(asDate(record.attendanceDate)) === key));
}
