...
export async function getActiveFeeTypes(schoolId: number) {
  ...
  return feeTypes.filter(
    (feeType) => feeType.status === "ACTIVE",
  );
}

export async function getFeeTypeById(
  schoolId: number,
  feeTypeId: number,
) {
  requireValidId(schoolId, "school ID");
...
export async function createFeeType(
  ...
  if (normalizedName.length > 150) {
    throw new Error("Fee type name is too long.");
  }
...
export async function getAcademicSessionsForFees(
  schoolId: number,
) {
  requireValidId(schoolId, "school ID");
...
export async function getClassesForSession(
  ...
  return classes
    ...
    .sort((a, b) => {
      const nameA =
        `${a.name} ${a.section ?? ""}`.trim();

      const nameB =
        `${b.name} ${b.section ?? ""}`.trim();
...
export async function createFeeAssignment(input: {
  ...
  if (input.termId !== undefined) {
    requireValidId(input.termId, "term ID");
  }

  if (input.classId !== undefined) {
    requireValidId(input.classId, "class ID");
  }
  ...
  if (!feeType) {
    throw new Error(
      "Fee type not found in this school.",
    );
  }

  if (feeType.status !== "ACTIVE") {
    throw new Error(
      "This fee type is inactive.",
    );
  }

  const sessions =
    await getAcademicSessionsForFees(
      input.schoolId,
    );
  ...
  if (input.termId !== undefined) {
    ...
    if (
      !terms.some(
        (term) => term.id === input.termId,
      )
    ) {
      throw new Error(
        "Term does not belong to the selected session.",
      );
    }
  }
  ...
  if (input.classId !== undefined) {
    ...
    if (
      !classes.some(
        (schoolClass) =>
          schoolClass.id === input.classId,
      )
    ) {
      throw new Error(
        "Class does not belong to the selected school and session.",
      );
    }
  }
  ...
  if (duplicate) {
    throw new Error(
      "This fee has already been assigned to the student for the selected session/term.",
    );
  }
...
export async function getStudentPayments(
  ...
  return payments
    ...
    .sort(
      (a, b) =>
        b.paymentDate.getTime() -
        a.paymentDate.getTime(),
    );
}
...
export async function getStudentFinancialSummary(
  ...
  const assignments =
    await getStudentFeeAssignments(
      schoolId,
      studentId,
    );
...
export async function createPayment(input: {
  ...
  if (!cashier) {
    throw new Error(
      "Cashier is not an active user in this school.",
    );
  }
